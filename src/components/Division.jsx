...
export default function Division({ side, image, alt, author, texts }) {
    ...
    return (
        <>
            <section id="outer-container" className={`w-full flex flex-col ${side === "right" ? "md:flex-row-reverse" : "md:flex-row"}`}>
                <img src={image} loading="lazy" alt={alt} className="w-full h-64 object-cover sm:h-80 md:h-auto md:w-1/2" title={`Photo by ${author} on Unsplash`}/>
